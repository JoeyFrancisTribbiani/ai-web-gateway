...
export default {
  ...
  async sendPrompt(page, prompt, selectors) {
    const inputSel = selectors.input || "textarea[placeholder*='输入']"
    await page.waitForSelector(inputSel, { timeout: 15000 })
    await page.click(inputSel)
    await page.keyboard.insertText(prompt)
    await page.waitForTimeout(500)

    const sendSel = selectors.sendButton || "button[aria-label*='发送']"
  ...
  async streamResponse(page, onChunk, selectors, signal) {
    const assistantSel = selectors.assistantMessage || "[data-testid='receive_message']"
    let lastText = ''
    let stableCount = 0
    ...
    while (!signal?.aborted) {
      ...
      const stopSel = selectors.stopButton || "button[aria-label*='停止']"
